...
* **Description:** Write a SQL script that lists all shows contained in `hbtn_0d_tvshows` without a genre linked. Displaying `tv_shows.title` and `tv_show_genres.genre_id` (which will be `NULL`). Sorted in ascending order by `tv_shows.title` and `tv_show_genres.genre_id` using a single `SELECT` statement with `LEFT JOIN` and `WHERE IS NULL`.
...
* **Directory:** `SQL_more_queries`
* **Description:** Write a SQL script that lists all genres of the show *Dexter* from the `hbtn_0d_tvshows` database using a single `SELECT` statement with `JOIN` operations across three tables (`tv_genres`, `tv_show_genres`, and `tv_shows`). Results are displayed under `tv_genres.name` and sorted in ascending order by the genre name.
...
* **Directory:** `SQL_more_queries`
* **Description:** Write a SQL script that lists all shows, and all genres linked to that show, from the database `hbtn_0d_tvshows`. If a show doesn't have a genre, displays `NULL` in the genre column. Uses a single `SELECT` statement with multiple `LEFT JOIN` operations across three tables (`tv_shows`, `tv_show_genres`, and `tv_genres`). Results display `tv_shows.title` and `tv_genres.name`, sorted in ascending order by show title and genre name.
